...
app.layout = \
    html.Div(children=[
        ...
        html.Div(children=[
                ...
                html.Div(
                    id='ticker_name',
                    className='three columns', style={'height': '100%', 'overflow-wrap': 'break-word'},
                ),
                html.Div(
...
def update_title(input_):
    try:
        if len(input_) == 3:
            ...
            return [html.H1(input_),
                    html.H2(ticker_info['Tên công ty'])]
...
def make_slider_properties(_input):
    _mark = pd.read_csv(root.loc['temp_', 'Path'], index_col=[0,1]).columns
    _min = 0
    _max = len(_mark)
